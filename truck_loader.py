...
# truck_loader.py
# load trucks based on given constraints before implementing delivery algorithm
import truck
...
class TruckLoader:
    def __init__(self, hash_table, distance_matrix, address_list):
        ...
        self.trucks = {1: Truck(1),
                       2: Truck(2),
                       3: Truck(3)}

        # initialize address_to_index
        self.address_to_index = self.create_address_to_index(address_list)
        ...
        self.optimizer = SimulatedAnnealing(distance_matrix, init_temp=10000, alpha=0.995,
    ...
    def load_initial_trucks(self, tl):
        ...
        for package_id in initial_packages:
            package = tl.hash_table.lookup(str(package_id))
            if package:
    ...
    def load_truck_2_packages(self, tl):
        ...
        for package_id in truck_2_packages:
            # cast package_id to string to ensure lookup function works as expected
            package = tl.hash_table.lookup(str(package_id))
            if package:
    ...
    def optimize_truck_routes(self, tl):
        ...
        optimizer = SimulatedAnnealing(self.distance_matrix, init_temp=10000, alpha=0.995,
                                       address_to_index=self.address_to_index)
    ...
    def create_address_to_index(self, address_list):
